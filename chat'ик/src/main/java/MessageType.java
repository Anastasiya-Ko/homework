package main.java;// Enum, который отвечает за тип сообщений между клиентом и сервером
/*
Основные моменты протокола:
- Когда новый клиент хочет подсоединиться к серверу, сервер должен запросить имя клиента.
- Когда клиент получает запрос имени от сервера он должен отправить своё имя серверу.
